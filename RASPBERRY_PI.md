...
看板列表頁本來就要抓，成本固定。**只有標題命中的文章才會額外去抓單篇內文**，
沒命中的連碰都不碰——這才是省下來的大宗。

有沒有設價格上限，決定的是命中之後怎麼處理，不是要不要分析：

| | 有上限 | 沒上限 |
|---|---|---|
| 讀內文、呼叫 API | 是 | 是 |
| 超過上限的文章 | 不通知 | 照通知 |
| 已售出／徵求文 | 不通知 | 照通知，但不標價格 |
| 通知標題 | `[36,000] …` ＋行情對照 | 同左（抽不到價格時維持原標題） |
...
輸入 `邀請碼` 後，回覆的**整則訊息就是邀請函**，直接轉發給對方即可，不用改字：
...
開通步驟：
1. 開啟 https://t.me/ptt_alertor_wells_bot
2. 點「開始」或輸入 /start
3. 把下面這組邀請碼貼給它
...
訊息裡**沒有任何對管理員說的話**——備註不會出現在這裡（它只在
`邀請碼 清單` 看得到），否則轉發出去對方會看到你怎麼稱呼他。

bot 連結是啟動時由 Telegram 通道回報自己的 username 填入的，換 bot 不用改程式。
